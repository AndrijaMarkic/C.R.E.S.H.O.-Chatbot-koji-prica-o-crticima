...
for line in open('databases/My Little Pony Friendship is Magic/mlp q - dub2.txt', encoding='utf8'):
    labels.append(line.strip().split(' ')[-1])
    questions.append(' '.join(line.split(' ')[:-1]))

answers = []

for line in open('databases/My Little Pony Friendship is Magic/mlp a - dub2.txt', encoding='utf8'):
...
def start_chat_mlp_dub2():
    ...
    while True:
        ...
        if user_response=='exit' or user_response=='quit':
            ...
            winsound.PlaySound(None, winsound.SND_PURGE)
            print('\n Puštam MLP 2nd Uvodnu Špicu sa Youtube.com\n ')
            hrvatski_voice('Puštam Moj Mali Poni drugu Uvodnu Špicu sa jutjuba')
            time.sleep(3)
            winsound.PlaySound('intros/mlp_g04_intro_dub2.wav', winsound.SND_ASYNC)
